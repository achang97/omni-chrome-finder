...
const CONTROL_BAR_COMPONENT_MAP = {
  ...
  PlaybackRateMenuButton: (
    <PlaybackRateMenuButton rates={[1, 1.25, 1.5, 2]} key="playback-rate" order={4} />
  ),
  FullscreenToggle: <FullscreenToggle key="fullscreen-toggle" order={5} />
};

const CONTROL_BAR_COMPONENT_LIST = [
  'PlayToggle',
  'ReplayControl',
  'ForwardControl',
  'VolumeMenuButton',
        ...
        <BigPlayButton
          ...
          className={s(
            `video-player-big-play-button ${
              player.hasStarted ? 'video-player-big-play-button-hide' : ''
            }`
          )}
        />
        <ControlBar
          ...
          autoHide={false}
          className={s('video-player-control-bar')}
        >
